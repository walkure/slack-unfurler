...
func fetchGrokStatus(id string) (*slack.Attachment, error) {
	return fetchAPI("GrokShare", loginApiID, func(graphQuery url.Values) {
		...
		return extractGrokShare(responseBody)
	})
}

func extractGrokShare(responseBody io.Reader) (*slack.Attachment, error) {

	grokShareContainer := grokEntity{}

	if err := json.NewDecoder(responseBody).Decode(&grokShareContainer); err != nil {
